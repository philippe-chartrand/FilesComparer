...
def move(moved, dir_one_path, dir_two_path, dir_two, execute_now, print_action=True):
    actions = []
    for k in sorted(moved.keys()):
        try:
            d1, d2, dest_dir, new_path, old_path = move_prepare_one(dir_one_path, dir_two_path, k, moved, print_action)
            if execute_now:
...
def move_prepare_one(dir_one_path, dir_two_path, k, moved, print_action):
    d1 = moved[k][0]
    d2 = moved[k][1]
...
def add(to_add, dir_one_path, dir_two_path, dir_two, execute_now, print_action=True):
    actions = []
    for k in sorted(to_add):
        try:
            dest_dir, d = add_prepare_one(dir_one_path, dir_two_path, k, to_add, print_action)
            if execute_now:
...
def add_prepare_one(dir_one_path, dir_two_path, k, to_add, print_action):
    d = to_add[k]
...
def update(changed, dir_two, execute_now, print_action=True):
    actions = []
    for k in sorted(changed.keys()):
        try:
            source, destination = update_prepare_one(changed, k, print_action)
            if execute_now:
...
def update_prepare_one(changed, k, print_action):
...
def restore(changed, dir_one, execute_now, print_action=True):
    actions = []
    for k in sorted(changed.keys()):
        try:
            source, destination = restore_prepare_one(changed, k, print_action)
            if execute_now:
...
def restore_prepare_one(changed, k, print_action):
    source = changed[k][0]['path']
    destination = changed[k][1]['path']
    if print_action:
        if changed[k][0]['mtime'] != changed[k][1]['mtime']:
...
def remove(deleted, dir_two, execute_now, print_action=True):
    actions = []
